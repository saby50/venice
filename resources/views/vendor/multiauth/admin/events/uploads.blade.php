...
<div class="panel-content">
  <form action="{{ URL::to('admin/events/uploadr') }}" method="post">
...
$("#fileuploader3").uploadFile({
  ...
  deleteCallback: function (data, pd) {
    for (var i = 0; i < data.length; i++) {
        $.post("<?= URL::to('admin/events/delete_banner/') ?>",  {op: "delete",name: data[i], id : "<?= Request::segment(4) ?>","_token":"{{ csrf_token()}}"},
...
$("#fileuploader5").uploadFile({
  ...
  deleteCallback: function (data, pd) {
    for (var i = 0; i < data.length; i++) {
        $.post("<?= URL::to('admin/events/delete_mobile_banner/') ?>",  {op: "delete",name: data[i], id : "<?= Request::segment(4) ?>","_token":"{{ csrf_token()}}"},
            function (resp,textStatus, jqXHR) {
                //Show Message
                alert("File Deleted");
            });
 ...
 url:"<?= URL::to('admin/events/upload_gallery') ?>",
  ...
  deleteCallback: function (data, pd) {
    for (var i = 0; i < data.length; i++) {
        $.post("<?= URL::to('admin/events/delete_gallery/') ?>",  {op: "delete",name: data[i], id : "<?= Request::segment(4) ?>","_token":"{{ csrf_token()}}"},
            function (resp,textStatus, jqXHR) {
                //Show Message
                alert("File Deleted");
            });
