...
@section('content')
<form action="{{ URL::to('admin/updatemenuitem') }}" method="post">
	@csrf
...
<script type="text/javascript">
  ...
  maxFileCount:1,
  fileName:"myfile",
  formData: {"_token":"{{ csrf_token()}}", 'id': "<?= $item_id ?>"},
  acceptFiles:"image/*",
  showDelete: true,
  returnType: "json",
  showDownload:false,
  ...
  deleteCallback: function (data, pd) {
    for (var i = 0; i < data.length; i++) {
        $.post("<?= URL::to('admin/menu/delete_featured_item/') ?>",  {op: "delete",name: data[i], id : "<?= $item_id ?>","_token":"{{ csrf_token()}}"},
